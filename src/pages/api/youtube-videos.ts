...
export default async function handler(
  ...
  try {
    // Use eventType=completed to get past live stream recordings (not active/upcoming streams)
    ...
    const ytRes = await fetch(url.toString(), { headers });
    if (!ytRes.ok) {
      const detail = await ytRes.text();
      return res.status(502).json({ error: "YouTube request failed", status: ytRes.status, detail });
    }
